...
export default function ServicePulse() {
  ...
  const handleSubmit = () => {
    if (reportData.type && reportData.ward && reportData.desc) {
      ...
      setTimeout(() => {
        ...
        setPhotos([]); setActiveTab("dashboard");
      }, 3000);
    }
  };
  ...
  const navItems = [
    ...
    ...(view === "official"
      ? [{ id:"analytics", icon:"▦", label:"Analytics" }, { id:"manage", icon:"≡", label:"Manage" }]
      : [{ id:"track",     icon:"◉", label:"Track" }]),
  ];

  // ─── shared styles ────────────────────────────────────────────────────────
  const card = { background:"#0f172a", border:"1px solid #1e293b", borderRadius:12, padding:"16px" };
  ...
  return (
    <div style={{
      ...
      paddingBottom: isMobile ? 64 : 0,   // room for bottom nav
    }}>

      {/* ── HEADER ── */}
      <header style={{
        borderBottom:"1px solid #1e2d4a", padding: isMobile ? "0 14px" : "0 24px",
        display:"flex", alignItems:"center", justifyContent:"space-between",
        height: isMobile ? 52 : 60,
        position:"sticky", top:0, background:"#0a0e1a", zIndex:50,
      }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            <div style={{ fontSize: isMobile ? 13 : 15, fontWeight:700, color:"#f1f5f9" }}>
        ...
        {/* view toggle */}
        <div style={{ display:"flex", background:"#111827", borderRadius:8, padding:3, border:"1px solid #1e2d4a", gap:2 }}>
          {["resident","official"].map(v => (
            <button key={v} onClick={() => { setView(v); setActiveTab("dashboard"); }} style={{
            ...
            }}>{v==="official" ? (isMobile ? "🔒" : "🔒 Official") : (isMobile ? "👤" : "👤 Resident")}</button>
          ))}
        </div>
      </header>
      ...
      <div style={{
        ...
        padding: isMobile ? "7px 14px" : "8px 24px",
        display:"flex", alignItems:"center", gap:12, flexWrap:"wrap",
      }}>
        <span style={{ fontSize:11, color:"#f59e0b", fontWeight:700, fontFamily:"monospace" }}>
          ⚡ ESKOM STAGE {loadshedding.stage}
        </span>
        <span style={{ fontSize: isMobile ? 11 : 12, color:"#d97706" }}>
          Next: <strong style={{ color:"#fbbf24" }}>{loadshedding.next}</strong>
          {!isMobile && ` · ${loadshedding.area}`}
        </span>
        {!isMobile && <span style={{ fontSize:11, color:"#92400e" }}>Tomorrow: {loadshedding.tomorrow}</span>}
        <span style={{ marginLeft:"auto", fontSize:10, color:"#78716c", fontFamily:"monospace" }}>EskomSePush</span>
      </div>
          ...
          <nav style={{
            ...
            {navItems.map(item => (
              <button key={item.id} onClick={() => setActiveTab(item.id)} style={{
                ...
                fontSize:13, fontWeight: activeTab===item.id ? 600 : 400, transition:"all 0.15s",
              }}>
        ...
        <main style={{ flex:1, padding: isMobile ? "16px 14px" : "24px", overflowY:"auto" }}>
          ...
          {activeTab === "report" && (
            <div style={{ maxWidth:560 }}>
              <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight:700, color:"#f1f5f9", margin:"0 0 4px" }}>Report an Issue</h1>
              <p style={{ fontSize:12, color:"#64748b", margin:"0 0 22px" }}>Help your community by reporting service delivery problems.</p>

              {submitted ? (
                <div style={{ ...card, textAlign:"center", padding:32, background:"#052e16", borderColor:"#16a34a" }}>
                ...
                <div>
                  ...
                  <div style={{ display:"flex", gap:6, marginBottom:24, alignItems:"center" }}>
                    {[1,2,3,4].map(step => (
                      <div key={step} style={{ display:"flex", alignItems:"center", gap:6 }}>
                        ...
                        {step<4 && <div style={{ width: isMobile ? 18 : 28, height:2, background: reportStep>step ? "#1d4ed8" : "#1e293b" }}></div>}
                      </div>
                    ))}
                  ...
                  {reportStep===1 && (
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                      {services.map(svc => (
                        <button key={svc.id}
                          onClick={() => { setReportData({...reportData, type:svc.id}); setReportStep(2); }}
                          style={{
                            background:"#0f172a",
                            border:`2px solid ${reportData.type===svc.id ? "#3b82f6" : "#1e293b"}`,
                            borderRadius:12, padding: isMobile ? "16px 12px" : "20px 16px",
                            cursor:"pointer", textAlign:"center",
                          }}>
                          <div style={{ fontSize: isMobile ? 28 : 32, marginBottom:8 }}>{svc.icon}</div>
                          <div style={{ fontSize: isMobile ? 12 : 13, fontWeight:600, color:"#e2e8f0" }}>{svc.label}</div>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {reportStep===2 && (
                    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                      <div>
                        <label style={{ fontSize:13, color:"#94a3b8", display:"block", marginBottom:7 }}>Select your ward</label>
                        <select value={reportData.ward} onChange={e => setReportData({...reportData, ward:e.target.value})}
                          style={{ width:"100%", padding:"11px 14px", background:"#0f172a", border:"1px solid #1e293b", borderRadius:8, color:"#e2e8f0", fontSize:13, outline:"none" }}>
                          <option value="">Choose ward...</option>
                          {WARDS.map(w => <option key={w} value={w}>{w}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={{ fontSize:13, color:"#94a3b8", display:"block", marginBottom:7 }}>Street / Location</label>
                        <input placeholder="e.g. Corner of Murchison & Lyell St"
                          style={{ width:"100%", padding:"11px 14px", background:"#0f172a", border:"1px solid #1e293b", borderRadius:8, color:"#e2e8f0", fontSize:13, outline:"none", boxSizing:"border-box" }} />
                      </div>
                      <div>
                        <label style={{ fontSize:13, color:"#94a3b8", display:"block", marginBottom:7 }}>Describe the problem</label>
                        <textarea rows={4} value={reportData.desc} onChange={e => setReportData({...reportData, desc:e.target.value})}
                          placeholder="What exactly is the issue? How long has it been happening?"
                          style={{ width:"100%", padding:"11px 14px", background:"#0f172a", border:"1px solid #1e293b", borderRadius:8, color:"#e2e8f0", fontSize:13, outline:"none", resize:"vertical", boxSizing:"border-box" }} />
                      </div>
                      <div style={{ display:"flex", gap:10 }}>
                        <button onClick={() => setReportStep(1)} style={{ flex:1, padding:"11px", background:"#1e293b", border:"none", borderRadius:8, color:"#94a3b8", fontSize:13, cursor:"pointer" }}>← Back</button>
                        <button onClick={() => reportData.ward && reportData.desc && setReportStep(3)}
                          style={{ flex:2, padding:"11px", background:"#1d4ed8", border:"none", borderRadius:8, color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer" }}>Continue →</button>
                      </div>
                    </div>
                  )}
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                      <div>
                        <label style={{ fontSize:13, color:"#94a3b8", display:"block", marginBottom:4 }}>
                          Add Photos <span style={{ color:"#475569" }}>(optional — up to 3)</span>
                        </label>
                        <p style={{ fontSize:12, color:"#475569", margin:"0 0 12px" }}>
                          A photo helps the team understand and prioritise the issue faster.
                        </p>
                        <label htmlFor="photo-upload" style={{
                          display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                          border:"2px dashed #1e293b", borderRadius:12,
                          padding: isMobile ? "22px 16px" : "28px 20px",
                          cursor:"pointer", background:"#0f172a", marginBottom:12,
                        }}>
                          <div style={{ fontSize:36, marginBottom:10 }}>📷</div>
                          <div style={{ fontSize:13, fontWeight:600, color:"#94a3b8", marginBottom:4 }}>
                            {isMobile ? "Tap to take a photo or upload" : "Click to upload a photo"}
                          </div>
                          ...
                          <input id="photo-upload" type="file" accept="image/*" capture="environment"
                            multiple onChange={handlePhotoUpload} style={{ display:"none" }} />
                        </label>
                          ...
                          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                            {photos.map((photo,i) => (
                              <div key={i} style={{ position:"relative", borderRadius:8, overflow:"hidden", border:"1px solid #1e293b" }}>
                                <img src={photo.url} alt="" style={{ width:"100%", height:80, objectFit:"cover", display:"block" }} />
                            ...
                            {photos.length < 3 && (
                              <label htmlFor="photo-upload" style={{
                                height:80, border:"2px dashed #1e293b", borderRadius:8,
                                display:"flex", alignItems:"center", justifyContent:"center",
                      ...
                      <div style={{ display:"flex", gap:10 }}>
                        <button onClick={() => setReportStep(2)} style={{ flex:1, padding:"11px", background:"#1e293b", border:"none", borderRadius:8, color:"#94a3b8", fontSize:13, cursor:"pointer" }}>← Back</button>
                        <button onClick={() => setReportStep(4)} style={{ flex:2, padding:"11px", background:"#1d4ed8", border:"none", borderRadius:8, color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer" }}>
                          {photos.length > 0 ? `Continue with ${photos.length} photo${photos.length>1?"s":""} →` : "Skip & Continue →"}
                        </button>
                      </div>
                    </div>
                  )}
                  ...
                  {reportStep===4 && (
                    <div>
                      <div style={{ ...card, marginBottom:16 }}>
                        <div style={{ fontSize:12, color:"#64748b", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.08em" }}>Review your report</div>
                        {[
                          ...
                          <div key={f.label} style={{ display:"flex", gap:12, marginBottom:10 }}>
                            <span style={{ fontSize:12, color:"#475569", width:76, flexShrink:0 }}>{f.label}</span>
                      ...
                      <div style={{ display:"flex", gap:10 }}>
                        <button onClick={() => setReportStep(3)} style={{ flex:1, padding:"11px", background:"#1e293b", border:"none", borderRadius:8, color:"#94a3b8", fontSize:13, cursor:"pointer" }}>← Back</button>
          ...
          {activeTab === "map" && (
            <div>
              <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight:700, color:"#f1f5f9", margin:"0 0 4px" }}>Ward Issue Map</h1>
              <p style={{ fontSize:12, color:"#64748b", margin:"0 0 16px" }}>Ladysmith / Alfred Duma — live report density by ward</p>
              <div style={{
                ...
                {[
                  { label:"W3", x:30, y:40, size:isMobile?42:52, color:"#ef4444", count:91 },
                  { label:"W5", x:58, y:55, size:isMobile?36:44, color:"#f59e0b", count:43 },
                  { label:"W2", x:45, y:25, size:isMobile?30:38, color:"#f59e0b", count:67 },
                  { label:"W7", x:70, y:30, size:isMobile?20:24, color:"#22c55e", count:12 },
                  { label:"W1", x:20, y:65, size:isMobile?24:30, color:"#f59e0b", count:28 },
                  { label:"W6", x:80, y:65, size:isMobile?18:20, color:"#22c55e", count:8  },
                ].map(ward => (
                  <div key={ward.label} style={{
                    ...
                    display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
                    cursor:"pointer", boxShadow:`0 0 ${ward.size/2}px ${ward.color}30`,
                  }}>
                ...
                <div style={{ position:"absolute", top:12, left:12, fontSize:10, color:"#334155", fontFamily:"monospace" }}>
                  LADYSMITH · 27 WARDS
                </div>
              </div>

              <div style={{ display:"grid", gridTemplateColumns: isMobile ? "repeat(4,1fr)" : "repeat(4,1fr)", gap:10, marginTop:14 }}>
                {WARDS.map((w,i) => {
                  const counts = [91,67,43,28,12,8,15,34];
                  const colors = ["#ef4444","#f59e0b","#f59e0b","#f59e0b","#22c55e","#22c55e","#22c55e","#f59e0b"];
                  return (
                    <div key={w} style={{ background:"#0f172a", border:`1px solid ${colors[i]}30`, borderRadius:8, padding:"10px 10px" }}>
                      <div style={{ fontSize:11, fontWeight:600, color:"#94a3b8" }}>{w}</div>
          ...
          {activeTab === "track" && (
            <div style={{ maxWidth:500 }}>
              <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight:700, color:"#f1f5f9", margin:"0 0 4px" }}>Track Your Report</h1>
              <p style={{ fontSize:12, color:"#64748b", margin:"0 0 20px" }}>Enter your reference number to check the status.</p>
              <div style={{ display:"flex", gap:10, marginBottom:20 }}>
                ...
                <button style={{ padding:"11px 18px", background:"#1d4ed8", border:"none", borderRadius:8, color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer" }}>Search</button>
              </div>
              <div style={{ ...card }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16 }}>
                  ...
                  <span style={{ fontSize:11, fontWeight:700, padding:"3px 10px", borderRadius:20, background:"#f59e0b20", color:"#f59e0b", border:"1px solid #f59e0b40", height:"fit-content" }}>In Progress</span>
                </div>
                {[
          ...
          {activeTab === "analytics" && view === "official" && (
            <div>
              <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight:700, color:"#f1f5f9", margin:"0 0 4px" }}>Analytics</h1>
              <p style={{ fontSize:12, color:"#64748b", margin:"0 0 20px" }}>Service delivery performance — April 2026</p>
              ...
              <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: isMobile ? 10 : 14, marginBottom:20 }}>
                {[
                  ...
                  { label:"Overdue >7d",    value:"31",  delta:"+5",   up:false },
                ].map(kpi => (
                  <div key={kpi.label} style={{ ...card }}>
                    <div style={{ fontSize:10, color:"#64748b", textTransform:"uppercase", letterSpacing:"0.08em" }}>{kpi.label}</div>
                    <div style={{ fontSize: isMobile ? 22 : 26, fontWeight:700, color:"#f1f5f9", margin:"6px 0 4px" }}>{kpi.value}</div>
              ...
              <div style={{ ...card, marginBottom:14 }}>
                <div style={{ fontSize:12, fontWeight:600, color:"#94a3b8", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.05em" }}>Reports by Category</div>
                {services.map((svc,idx) => {
                  const pct = [43,91,67,12][idx];
                  return (
                    <div key={svc.id} style={{ marginBottom:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                        <span style={{ fontSize:12, color:"#94a3b8" }}>{svc.icon} {svc.label}</span>
                        <span style={{ fontSize:12, color:"#64748b", fontFamily:"monospace" }}>{pct}</span>
                      </div>
                      <div style={{ background:"#1e293b", borderRadius:4, height:8 }}>
                        <div style={{ width:`${(pct/91)*100}%`, height:"100%", borderRadius:4, background:STATUS[svc.status].color }}></div>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ ...card, overflowX:"auto" }}>
                <div style={{ fontSize:12, fontWeight:600, color:"#94a3b8", marginBottom:14, textTransform:"uppercase", letterSpacing:"0.05em" }}>SLA by Ward</div>
                <table style={{ width:"100%", borderCollapse:"collapse", minWidth:300 }}>
                  <thead>
                    <tr style={{ borderBottom:"1px solid #1e293b" }}>
                      {["Ward","Reports","Resolved","Avg","SLA"].map(h => (
                        <th key={h} style={{ textAlign:"left", padding:"6px 8px", fontSize:10, color:"#475569", textTransform:"uppercase" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[["Ward 3",91,34,5.2,"warning"],["Ward 2",67,41,3.8,"good"],["Ward 5",43,20,6.1,"critical"],["Ward 1",28,18,2.9,"good"],["Ward 8",34,22,4.0,"good"]].map(([ward,rep,res,days,sla]) => (
                      <tr key={ward} style={{ borderBottom:"1px solid #0f172a" }}>
                        <td style={{ padding:"9px 8px", fontSize:13, color:"#e2e8f0" }}>{ward}</td>
                        <td style={{ padding:"9px 8px", fontSize:13, color:"#94a3b8", fontFamily:"monospace" }}>{rep}</td>
                        <td style={{ padding:"9px 8px", fontSize:13, color:"#94a3b8", fontFamily:"monospace" }}>{res}</td>
                        <td style={{ padding:"9px 8px", fontSize:13, color:"#94a3b8", fontFamily:"monospace" }}>{days}d</td>
                        <td style={{ padding:"9px 8px" }}>
                          <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:20, background:`${STATUS[sla].color}20`, color:STATUS[sla].color }}>{STATUS[sla].label}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {activeTab === "manage" && view === "official" && (
            <div>
              <h1 style={{ fontSize: isMobile ? 18 : 22, fontWeight:700, color:"#f1f5f9", margin:"0 0 4px" }}>Manage Reports</h1>
              <p style={{ fontSize:12, color:"#64748b", margin:"0 0 16px" }}>Review and update the status of all submitted reports.</p>
              <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
                {["All","Open","In Progress","Resolved"].map(f => (
                  <button key={f} style={{
                    padding:"6px 14px", borderRadius:20, border:"1px solid #1e293b",
                    background: f==="All" ? "#1d4ed8" : "#0f172a",
                    color: f==="All" ? "#fff" : "#64748b", fontSize:12, cursor:"pointer",
                  }}>{f}</button>
                ))}
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {recentReports.map(r => (
                  <div key={r.id} style={{ ...card, padding:"12px 14px", display:"flex", alignItems: isMobile ? "flex-start" : "center", gap:12, flexDirection: isMobile ? "column" : "row" }}>
                    <div style={{ flex:1 }}>
      ...
      {isMobile && (
        <nav style={{
          ...
          background:"#080c17", borderTop:"1px solid #1e2d4a",
          ...
          {navItems.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} style={{
              ...
              background: activeTab===item.id ? "#0f1a2e" : "transparent",
              color: activeTab===item.id ? "#93c5fd" : "#475569",
              fontSize:10, fontWeight: activeTab===item.id ? 600 : 400,
              borderTop: activeTab===item.id ? "2px solid #3b82f6" : "2px solid transparent",
